...
// ../.wrangler/tmp/bundle-MXVt3Q/checked-fetch.js
...
var onRequestPost = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    const match2 = cookieHeader.match(/cms_session=([^;]+)/);
    const token = match2 ? match2[1] : null;
    if (!token) {
    ...
    const secretStr = env.AUTH_SECRET || "change-me-in-env";
    ...
    const { file_name, mime_type, file_content, size_bytes } = body;
    if (!file_content || !file_name || !mime_type) {
    ...
    const base64Data = file_content.includes(",") ? file_content.split(",")[1] : file_content;
    const fileBuffer = base64ToArrayBuffer(base64Data);
    await env.BUCKET.put(key, fileBuffer, {
      httpMetadata: { contentType: mime_type }
    });
    let baseUrl = "https://pub-844c31a0552a92bec753504a01b5f3bb.r2.dev";
    if (typeof process !== "undefined" && process.env && process.env.R2_PUBLIC_BASE_URL) {
    ...
    console.error("Functions Upload Error:", err);
...
// ../.wrangler/tmp/pages-XPAexJ/functionsRoutes-0.03307852856919302.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-MXVt3Q/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-MXVt3Q/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.5796349707166462.mjs.map
