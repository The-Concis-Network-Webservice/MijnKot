...
export default function AdminMediaPage() {
  ...
  const uploadAsset = async (file: File) => {
    ...
    try {
      ...
      if (!mediaRes.ok) throw new Error(payload.error || "Registratie mislukt.");
  ...
  const deleteAsset = async (id: string) => {
    ...
    const payload = await res.json();
    if (!res.ok) { push(payload.error ?? "Kan media niet verwijderen.", "error"); return; }
    setDeleteConfirmId(null);
    await loadAssets();
    push("Media verwijderd.");
  };
    ...
    <AdminGuard>
      <AdminShell>
        <PageHeader
          ...
          description="Alle geüploade afbeeldingen, herbruikbaar voor koten."
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 space-y-6">
          ...
          >
            {uploading ? (
              ...
              <div className="p-8 text-center space-y-2 pointer-events-none">
                ...
                <p className="text-sm font-medium text-text-main">Sleep afbeeldingen hierheen of klik om te uploaden</p>
          ...
          {loading ? (
            ...
            <p className="text-sm text-text-muted text-center py-6">Nog geen media geüpload.</p>
          ) : (
            <div className="grid md:grid-cols-3 gap-4">
              {assets.map((asset) => (
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {deleteConfirmId === asset.id ? (
                        <div className="flex items-center gap-2">
                          ...
                          <button type="button" onClick={() => deleteAsset(asset.id)} className="text-xs font-medium text-red-600 hover:text-red-700 cursor-pointer">Ja</button>
                          <button type="button" onClick={() => setDeleteConfirmId(null)} className="text-xs text-gray-500 hover:text-gray-700 cursor-pointer">Nee</button>
                        </div>
                      ) : (
                        <button
                          type="button"
