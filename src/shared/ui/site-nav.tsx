...
export function SiteNav({ 
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 bg-surface-card/95 backdrop-blur-sm border-b border-border-light transition-all duration-300 ${className}`}>
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <div className="relative group">
              ...
              <div className="absolute top-full left-0 w-48 py-2 mt-1 bg-surface-card border border-border-light rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform origin-top translate-y-2 group-hover:translate-y-0">
                {rentTypes.map((rt) => (
                  <Link key={rt.id} href={`/koten?type=${rt.slug}`} className="block px-4 py-2 text-sm text-primary-500 hover:bg-surface-subtle hover:text-primary-600">
                    {rt.name}
                  </Link>
                ))}
